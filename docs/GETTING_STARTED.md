...
For databases where an in-memory mirror is too large, use a provider-owned
SQLite file:

```csharp
connection.ConnectionString =
    "Data Source=C:\\data\\large.mdb;Read Only=true;Mirror Mode=file;"
    + "Mirror Folder=C:\\data\\mirrors";
```
...
Ordinary autocommit INSERT/UPDATE/DELETE statements use the same private staging
boundary. A numeric AutoNumber generated by INSERT is available through
`UCanAccessConnection.LastInsertedId`.
